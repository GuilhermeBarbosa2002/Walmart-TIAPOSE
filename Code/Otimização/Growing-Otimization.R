...
montecarlo_growing <- function(){
  ...
  plot_iteration_means(montecarlo_curve, N, "Montecarlo",median(montecarlo_values))
...
hill_climbing_growing <- function(){
  ...
  for (i in 1:length(grupos)) {
    ...
    start_index <- (i - 1) * N + 1
    end_index <- i * N
    hill_curve[start_index:end_index] <- curve
  ...
  plot_iteration_means(hill_curve, N, "Hill - Climbing",median(hill_climbing_values))
...
simulatedAnnealing_growing <- function(){
  ...
  for (i in 1:length(grupos)) {
    ...
    start_index <- (i - 1) * N + 1
    end_index <- i * N
    snn_curve[start_index:end_index] <- curve
  ...
  plot_iteration_means(snn_curve, N, "Simulated Annealing",median(simulatedAnnealing_values))
...
rgba_growing <- function(){
  ...
  plot_iteration_means(rgba_curve, N, "RGBA Genetic", median(rgba_values))
